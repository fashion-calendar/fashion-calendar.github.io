L.Control.SliderControl = L.Control.extend({
  options: {
    ...
    timeStrLength: 16, // the size of  yyyy-mm-dd hh:mm:ss - if millis are present this will be larger
    maxValue: -1,
    minValue: 0,
  ...
  onAdd: function(map) {
    ...
    $(sliderContainer).append('<div class="text-white bg-success py-3 px-2 mb-4 text-center" style="width:275px; border-radius:5px;"><h5>Date Range</h5><div id="slider-timestamp"></div></div><div id="leaflet-slider" style="width:275px"><div class="ui-slider-handle"></div></div>');
  ...
  startSlider: function() {
        ...
        if (!!_options.markers[ui.value]) {
          ...
          if (_options.markers[ui.value].feature !== undefined) {
            if (_options.markers[ui.value].feature.properties[_options.timeAttribute]) {
              if (_options.range) {
                $('#slider-timestamp').html(_extractTimestamp(_options.markers[ui.values[0]].feature.properties[_options.timeAttribute], _options) + ' - ' + _extractTimestamp(_options.markers[ui.values[1]].feature.properties[_options.timeAttribute], _options));
              } else {
    ...
    } else if (_options.range && _options.alwaysShowDate) {
      $('#slider-timestamp').html(_extractTimestamp(_options.markers[_options.minValue].feature.properties[_options.timeAttribute], _options) + ' - ' + _extractTimestamp(_options.markers[_options.maxValue].feature.properties[_options.timeAttribute], _options));
    }
